...
const Card = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-2xl border border-gray-100 bg-white text-gray-950 shadow-sm hover:shadow-md transition-shadow duration-300",
        className
      )}
      {...props}
    />
...
const CardFooter = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("flex items-center p-6 md:p-8 pt-0 md:pt-0 border-t border-gray-50/50 mt-4", className)}
      {...props}
    />
  )
);
