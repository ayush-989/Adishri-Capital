...
export function FileUpload({ label, error, onChange, accept = "image/*", className, ...props }: FileUploadProps) {
  ...
  return (
    <div className={cn("w-full", className)}>
      ...
      {preview ? (
        <div className="relative rounded-xl border-2 border-gray-200 border-dashed p-2 bg-gray-50/50">
          <img src={preview} alt="Preview" className="h-40 w-full object-contain rounded-lg" />
          <button
            type="button"
            onClick={handleClear}
            className="absolute top-3 right-3 p-1.5 bg-error text-white rounded-full hover:bg-red-600 shadow-md transition-all active:scale-95"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <label className={cn(
          ...
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            <UploadCloud className="w-8 h-8 mb-3 text-gray-400 group-hover:text-primary-500 transition-colors" />
      ...
      {error && <p className="mt-1.5 ml-0.5 text-xs font-medium text-error">{error}</p>}
    </div>
  );
}
