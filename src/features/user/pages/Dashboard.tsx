          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.id} className="flex flex-row md:flex-col items-center gap-4 md:gap-2 w-full md:w-32">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 border-white shadow-sm transition-colors duration-300 md:mx-auto ${isCompleted ? "bg-green-500 text-white" : isCurrent ? "bg-blue-500 text-white animate-pulse" : "bg-slate-200 text-slate-400"}`}>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <HandCoins className="text-blue-600" /> Loan Overview
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                      ...
                      <p className="text-2xl font-bold text-slate-900">{formatCurrency(calculateTotalPayable(application.finalAmount, application.finalInterestRate ?? 0))}</p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
