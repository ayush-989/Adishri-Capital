...
import { ADMIN_UPI_ID, LOAN_STATUS, ROUTES } from "../../utils/constants";
import { formatCurrency, calculateTotalPayable, generateTxnId } from "../../utils/helpers";
...
export function Dashboard() {
  ...
  const handleRepaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const screenshotUrl = await uploadFile(repayScreenshot, `repayments/${application.appId}/${txnId}`);
      await setDoc(doc(db, "transactions", txnId), {
        txnId, loanId: application.appId, amount: Number(repayAmount),
        utr: repayUtr.trim(), screenshotUrl, type: "repayment",
    ...
    if (status === LOAN_STATUS.REJECTED) {
      return (
        <div className="bg-red-50 p-5 rounded-2xl border border-red-200 text-center">
          ...
          <p className="text-red-500 text-sm mt-1">Your application doesn't meet current criteria.</p>
        </div>
      );
    }
    ...
    return (
      <div className="flex items-center justify-between gap-1 sm:gap-2 overflow-x-auto pb-1">
        {STATUS_STEPS.map((step, idx) => {
          ...
          return (
            <div key={step.id} className="flex flex-col items-center gap-1.5 min-w-[52px]">
  ...
  const outstanding = application?.remainingBalance !== undefined
    ? application.remainingBalance
    : (application?.finalAmount ? calculateTotalPayable(application.finalAmount, application.finalInterestRate) : 0);
    ...
    <MainLayout>
              ...
              <CardContent className="pt-6 pb-5">
                ...
                {application.status === LOAN_STATUS.PENDING_PAYMENT && (
                  <div className="mt-5 bg-amber-50 border border-amber-200 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    ...
                    <Link to={`${ROUTES.PROCESSING_FEE}?loanId=${application.appId}`} className="shrink-0">
                      <Button className="bg-amber-600 hover:bg-amber-700 text-white text-sm py-2 px-4">
                        Complete Payment
                      </Button>
                    </Link>
                  </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      { label: "Total Payable", value: formatCurrency(calculateTotalPayable(application.finalAmount, application.finalInterestRate)), color: "text-slate-900" },
                  ...
                  {application.status === LOAN_STATUS.DISBURSED && outstanding > 0 && (
                    <div className="mt-4 bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                      ...
                      <Button
                        ...
                        className="bg-emerald-600 hover:bg-emerald-700 group text-sm py-2.5 shrink-0 w-full sm:w-auto"
                      >
      ...
      >
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col items-center mb-4">
          ...
          >
            <span className="font-mono font-bold text-slate-900 text-sm">{ADMIN_UPI_ID}</span>
            <Copy size={14} className="text-blue-600" />
          </button>
        </div>
        <form id="repay-form" onSubmit={handleRepaymentSubmit} className="space-y-4">
          <Input label="Amount Paid (₹)" type="number" required value={repayAmount} onChange={(e) => setRepayAmount(e.target.value)} placeholder="Enter exact amount" min={1} />
          ...
          <p className="text-xs text-slate-500 bg-amber-50 border border-amber-100 rounded-xl p-3">
            ⚠️ Admin will verify your payment and update your outstanding balance automatically.
          </p>
        </form>
      </Modal>
    </MainLayout>
